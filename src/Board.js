...
class Board extends Component {
  ...
  flipCellsAround(coord) {
    ...
    // flip individual cell
    flipCell(y, x);

    // flip the y neighbors
    flipCell(y - 1, x);
    flipCell(y + 1, x);

    // flip the x neighbors
    flipCell(y, x - 1);
    flipCell(y, x + 1);
    ...
    // GCM: Research 'every';
    // for every row, and for every cell in every row, check to see if the cell is not false
  ...
  render() {
    // if this.state.hasWon is true, render "you won!" else, render the board
